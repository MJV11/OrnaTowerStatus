...
export interface EnhancedFloor extends Floor {
    nextfifty: String
    nextfiftyuntil: String 
}

// Type alias for an array of Floor objects
export type Floors = Array<Floor>

// CheckPointFloors represents checkpoints in tower growth or `false` if not valid.
type CheckPointFloors = {
    floors: Floor[]; // List of floors at this checkpoint
    time: Date; // Time of the checkpoint
} | false;
...
/**
 * Calculate the current floors of each tower.
 * @param { Date } time - The time to calculate tower floors for
 * @returns { Floors } - Array of Floor objects representing current tower states
 */
...
/**
 * Enhance the tower floor data with information about when they reach floor 50.
 * @param { Date } currentTime - The current time
 * @returns { EnhancedFloor[] } - Array of EnhancedFloor objects
 */

export function getEnhancedTowerFloors(currentTime: Date): EnhancedFloor[] {
  const floors = getTowerFloors(currentTime);

  return floors.map((tower) => {
    const checkpoints = getTowerFloorsInNextDays(currentTime, 7);

    let nextfifty: string | null = null;
    let nextfiftyuntil: string | null = null;

    for (let i = 0; i < checkpoints.length; i++) {
      const checkpoint = checkpoints[i];

      // Ensure checkpoint is valid
      if (checkpoint && checkpoint.floors) {
        const towerAtCheckpoint = checkpoint.floors.find((floor: Floor) => floor.kind === tower.kind);
        const hours = checkpoint.time.getHours();
        const minutes = String(checkpoint.time.getMinutes()).padStart(2, '0');
        const period = hours >= 12 ? 'PM' : 'AM';
        const formattedHours = hours % 12 === 0 ? 12 : hours % 12; // Convert 0 or 24 to 12

        const checkpointTime = `${checkpoint.time.getMonth() + 1}/${checkpoint.time.getDate()} ${formattedHours}:${minutes} ${period}`;

        if (towerAtCheckpoint && towerAtCheckpoint.floor === 50) {
          if (!nextfifty) {
            nextfifty = checkpointTime; // Set the first time the tower reaches floor 50
          }
        } else if (nextfifty) {
          nextfiftyuntil = checkpointTime; // Update to the last time it remains at floor 50
          break; // Stop searching once the tower is no longer at floor 50
        }
      }
    }
    ...
    return {
      ...tower,
      nextfifty: nextfifty || 'N/A',
      nextfiftyuntil: nextfiftyuntil || 'N/A',
    } as EnhancedFloor;
  });
}
